...
(let ((extension-path (format #f "~a/libgash.so" (getcwd)))
  ...
  (catch 'unbound-variable
    ...
    (lambda (key . args)
      ...
      (load (format #f "~a/history.scm" (getcwd))))))
...
;; this uses *this-executable-script* which is NOT what we want. 
;; (define (reload-gash)
;;   (let* ((port (open-file *this-executable-script* "r"))
;; 	 (str #f))
;;     (read-line port)
;;     (read-line port)
;;     (read-line port)
;;     (set! str (get-string-all port))
;;     (close-port port)
;;     (eval-string str)))
...
(define (main-entry ;; args
  ...
  ;; cant use *this-executable-script*
  ;; (if (char=? (string-ref (car args) 0) #\.)
  ;;     (set! *this-executable-script* (string-append (getcwd)
  ;; 						    (substring (car args) 1)))
  ;;     (set! *this-executable-script* (car args)))
  ;; (echo *this-executable-script*)
  (load (string-append home-directory "/.gashrc"))
  (display  "loaded ") (echof (string-append "#^2" home-directory "/.gashrc"))
